        ...
        findViewById<Button>(R.id.btnInsert).setOnClickListener {
            ...
            if (edBook.length() < 1 || edPrice.length() < 1 || edYear.length() < 1)
                ...
                try {
                    // 新增一筆書籍紀錄於myTable資料表
                    dbrw.execSQL(
                        "INSERT INTO myTable(brand, price, year) VALUES(?,?,?)",
                        arrayOf(edBook.text.toString(),
                            edPrice.text.toString(), edYear.text.toString())
                    )
        ...
        findViewById<Button>(R.id.btnUpdate).setOnClickListener {
            ...
            if (edBook.length() < 1 || edPrice.length() < 1)
                ...
                try {
                    ...
                    dbrw.execSQL("UPDATE myTable SET price = ${edPrice.text}, year = ${edYear.text} WHERE brand LIKE '${edBook.text}'")
                    showToast("更新:${edBook.text},價格:${edPrice.text},年份:${edYear.text}")
                    cleanEditText()
                } catch (e: Exception) {
                    showToast("更新失敗:$e")
                }
        ...
        findViewById<Button>(R.id.btnDelete).setOnClickListener {
            ...
            if (edBook.length() < 1)
                ...
                try {
                    ...
                    dbrw.execSQL("DELETE FROM myTable WHERE brand LIKE '${edBook.text}'")
